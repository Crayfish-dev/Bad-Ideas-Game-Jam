...
func interact(player: Player) -> void:
	...
	var is_last = current_page == dialogue_pages.size() - 1
	...
	if is_last:
		...
		tween.tween_property(self, "scale", Vector3(0,0,0), 1.0)
		await tween.finished
		player.play_dialogue("Jaffar (you)", ["... what? ", 'when i bought this place there wasnt any "book"...'])
		queue_free()
		return
