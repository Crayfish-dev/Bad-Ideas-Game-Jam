...
@export var item_spawning_point : Node3D
...
@onready var shop_ui: Control = $Neck/Camera/CanvasLayer/UI
@onready var quest_background: TextureRect = $Neck/Camera/CanvasLayer/CursorSprite/QuestBackground
...
func _physics_process(delta: float) -> void:
	quest_background.visible = not shop_ui.visible
...
func _update_cursor() -> void:
	...
	if collider is NPC:
		title.text = collider._name
		description.text = "talk to client [E]"
		cursor.frame = 4
		return

	if collider is Interactable:
		var obj : Interactable = collider
		title.text = obj._name
		description.text = obj._description
		cursor.frame = 4
		return

	if collider is Item:
		title.text = collider._name
		description.text = collider._description
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if Input.is_action_just_pressed("ui_cancel") and shop_ui.visible:
...
func _hundle_interactions():
	...
	if Input.is_action_just_pressed("interact"):
		if seeing_obj is ServiceBell:
			seeing_obj._press()
		if seeing_obj is BookOfKnoledge:
			seeing_obj.interact(self)
		if seeing_obj is NPC:
			seeing_obj.clicked(self)
		if seeing_obj is Shop:
			seeing_obj.use(self)
		if seeing_obj is RustRemover:
			if seeing_obj.opened:
				seeing_obj.open_close(0.0, false)
			else:
				seeing_obj.open_close(83.0, true)
...
func _show_dialogue_page(_name: String, dialogue: Array[String], index: int, speed: float) -> void:
	...
	var prev_visible = dialogue_label.visible_characters
	...
	while _dialogue_tween and _dialogue_tween.is_running():
		await get_tree().process_frame
		if not is_inside_tree():
			return
		var current = dialogue_label.visible_characters
		if current > prev_visible:
			for i in range(prev_visible, current):
				var c = line[i]
				if c != " " and c != "\n":
					dialogue_char_shown.emit()
