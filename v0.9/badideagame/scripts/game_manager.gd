...
func _turn_in_boxes(quest_name: String, boxes: Array) -> void:
	...
	for box in boxes:
		...
		if box is Box and box.closed:
			...
			if box is OrganicBox:

				for item in box.get_items():
					item.queue_free()

				box.bodies_inside.clear()
				box.deteriorate()
...
func _cleanup_quest_items(required: Array) -> void:
	var all_nodes = get_tree().get_nodes_in_group("items")
	_recursive_cleanup(get_tree().current_scene, required)

func _recursive_cleanup(node: Node, required: Array) -> void:
	for child in node.get_children():
		var src = child.get_meta("source_scene", null)
		if src != null and src in required:
			child.queue_free()
		else:
			_recursive_cleanup(child, required)
...
func _spawn_required_items(required: Array) -> void:

	for scene in required:
		if current_NPC.leaving:
			_cleanup_quest_items(required)
			return
		await get_tree().create_timer(0.5).timeout
		var i = scene.instantiate()
		if i._name.to_lower() == "flask":
			continue
