...
func _ready():
	var nor_scale : float = scale.y
	scale = Vector3.ZERO
	await get_tree().create_timer(randi_range(1, 8)).timeout
	var tween = create_tween()
	tween.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
	tween.tween_property(self, "scale", Vector3(nor_scale,nor_scale,nor_scale), 1.5)
...
func _start_talk_bob(pl: Player) -> void:
	if _talk_tween:
		_talk_tween.kill()
	...
	pl.dialogue_char_shown.connect(_on_char_shown)
	pl.dialogue_skipped.connect(_on_dialogue_skipped)

func _on_char_shown() -> void:
...
func _stop_talk_bob(pl: Player) -> void:
	if _talk_tween:
		_talk_tween.kill()
		_talk_tween = null
	if pl.dialogue_char_shown.is_connected(_on_char_shown):
		pl.dialogue_char_shown.disconnect(_on_char_shown)
	if pl.dialogue_skipped.is_connected(_on_dialogue_skipped):
		pl.dialogue_skipped.disconnect(_on_dialogue_skipped)
	var tween = create_tween()
...
func start_leaving() -> void:
	if leaving:
		return
	leaving = true
	var quest = GameManager.quests.get(GameManager.active_quest, {})
	var required = quest.get("required_items", [])
	GameManager._cleanup_quest_items(required)
	_leave_with_tween()
...
func _leave() -> void:
	...
	if instance._name == _name:
		instance._name += " again"
