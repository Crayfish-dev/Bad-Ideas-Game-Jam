...
@export var quest_time : float = 80.0
...
func clicked(pl : Player) -> void:
	...
	if not delivered:
		if late:
			await pl.play_dialogue(_name, late_dialogue)
		...
		if late:
			await pl.play_dialogue(_name, late_delivery_dialogue)
		else:
			await pl.play_dialogue(_name, delivery_dialogue)
...
func _leave_with_tween() -> void:
	...
	tween.tween_property(self, "position", leaving_point, 1.0)
	tween.finished.connect(_leave)

func _leave() -> void:
	var dir = DirAccess.open(NPC_FOLDER)
	if not dir:
		push_error("Cannot open folder: %s" % NPC_FOLDER)
		return
